...
export const restaurantService = {
  ...
  async getById(id: number): Promise<Restaurant> {
    const response = await apiClient.get<ApiResponse<Restaurant>>(
      `/api/admin-portal/restaurant/${id}`
    );
...
export const analyticsService = {
  ...
  async getOrders(
    ...
    const response =
      await apiClient.get<ApiResponse<PaginatedResponse<UserOrder>>>(endpoint);
    if (!response.data.data) {
  ...
  async getDashboardSummary(restaurantId: number) {
    const response = await apiClient.get(
      `/api/analytics/dashboard/summary/${restaurantId}`
    );
    return response.data;
  },

  // Obtener item más vendido
  async getTopSellingItem(
    filters: Pick<
      DashboardFilters,
    ...
    const response = await apiClient.get(
      `/api/analytics/dashboard/top-selling-item?${params.toString()}`
    );
    return response.data;
  },
};
