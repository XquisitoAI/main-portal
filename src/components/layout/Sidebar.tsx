...
const Sidebar: React.FC = () => {
  ...
  return <aside ref={sidebarRef} className={`bg-[#EAB3F4] text-[#0F172A] h-screen sticky top-0 left-0 border-r border-[rgba(0,0,0,0.06)] transition-all duration-200 ease-in-out ${expanded ? 'w-64' : 'w-[72px]'}`} onMouseEnter={() => setExpanded(true)} onMouseLeave={() => setExpanded(false)} onFocus={() => setExpanded(true)} tabIndex={0} aria-expanded={expanded}>
      <div className="p-4 flex justify-center">
        <div className="max-w-[64px] max-h-[64px] pt-4 pb-3">
          <img src="/iso-1-white.webp" alt="Xquisito" className="object-contain w-full h-full" />
        </div>
      </div>
        ...
        <ul className="space-y-2">
          {navItems.map(item => <li key={item.path}>
              <Link href={item.path} className={`flex items-center px-3 py-3 rounded-lg transition-all relative ${isActive(item.path) ? 'bg-[rgba(0,0,0,0.08)] text-[#0B1220] border-l-[3px] border-[#9A4BB8]' : 'hover:bg-[rgba(0,0,0,0.06)] hover:text-[#111827] border-l-[3px] border-transparent'}`} aria-current={isActive(item.path) ? 'page' : undefined}>
                ...
                {!expanded && <div className="absolute left-14 bg-gray-800 text-white px-2 py-1 rounded text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50">
                    {item.label}
                  </div>}
              </Link>
            </li>)}
