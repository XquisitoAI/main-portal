...
import { useVolumeTimeline, useOrdersTimeline, useTransactionsTimeline } from '../../hooks/useApiData';
...
const calculatePercentChange = (data: any[], metricType: 'currency' | 'number' | 'percent') => {
  if (data.length < 2) return 0;

  // Sumar Flex Bill y Tap Order & Pay para cada período
  const firstTotal = (data[0]['Flex Bill'] || 0) + (data[0]['Tap Order & Pay'] || 0);
  const lastTotal = (data[data.length - 1]['Flex Bill'] || 0) + (data[data.length - 1]['Tap Order & Pay'] || 0);
...
const DetailedMetricChart: React.FC<DetailedMetricChartProps> = ({
  ...
  const { data: rawTimelineData, isLoading, isError } = timelineHook({
    view_type: viewType,
  ...
  const timelineData = useMemo(() => {
    ...
    if (title === 'Ingresos Xquisito') {
      ...
      const rawTotal = rawTimelineData.reduce((sum: number, item: TimelineDataItem) =>
        sum + (item['Flex Bill'] || 0) + (item['Tap Order & Pay'] || 0), 0);
      console.log('  Total:', rawTotal);

      const processedTotal = processedData.reduce((sum: number, item: TimelineDataItem) =>
        sum + (item['Flex Bill'] || 0) + (item['Tap Order & Pay'] || 0), 0);
    ...
    if (rawTimelineData.length > 0) {
      ...
      const total = rawTimelineData.reduce((sum: number, item: TimelineDataItem) =>
        sum + (item['Flex Bill'] || 0) + (item['Tap Order & Pay'] || 0), 0);
        ...
        <div className="bg-white p-1.5 sm:p-2 border border-gray-200 shadow-sm rounded-md">
          ...
          {payload.map((entry: any, index: number) => (
            <div key={index} className="flex justify-between items-center text-[10px] sm:text-xs mb-1">
              ...
              <span className="font-medium ml-2">{formatValue(entry.value)}</span>
            </div>
          ))}
